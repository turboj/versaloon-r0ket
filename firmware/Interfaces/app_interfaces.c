...
void GPIO_SetMode(int GPIOx, uint8_t pin, uint8_t mode)
{
	// uint32_t tmp_reg;
        ...
        if (mode &3) {
          gpioSetDir(GPIOx, pin, 1);
        } else {
          //input, IOCON TODO
          gpioSetDir(GPIOx, pin, 0);
        }

        if (GPIOx==2) {

        	if (pin==11){
        		IOCON_PIO2_11=IOCON_PIO2_11_HYS_ENABLE | IOCON_PIO2_11_MODE_INACTIVE | 0xC0;
        	} else
        	if (pin==5) IOCON_PIO2_5=IOCON_PIO2_5_HYS_ENABLE | 0xC0;
        	else if (pin==8) IOCON_PIO2_8=IOCON_PIO2_8_MODE_PULLUP | 0xC0;

        } else if (GPIOx==0) {
        	if (pin==8) IOCON_PIO0_8=IOCON_PIO0_8_HYS_ENABLE | IOCON_PIO0_8_MODE_PULLDOWN | 0xC0;
        	else if (pin==9)   IOCON_PIO0_9=IOCON_PIO0_9_MODE_PULLDOWN| 0xC0;
        }
...
static vsf_err_t delay_delayus(uint16_t us)
{
	us &= 0x7FFF;
        ...
        uint32_t delay = us * (((72000000UL/ 100) / 45)/1000); // Release Mode (-Os)
        while (delay > 0)
        {
          __asm volatile ("nop");
          delay--;
...
{
	return 0;// core_interfaces.tickclk.init();
}
